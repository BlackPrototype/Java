 ...
 * @author Prototype_
 */
@WebServlet(name = "alapServlet", urlPatterns = {"/alapServlet"})
public class alapServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        if((user_Name.isEmpty()) || !(user_Name.equals(uName))){                
            String dataInput = "INSERT INTO USER1.PERSONAL_INFO (USERNAME, BIRTH_DATE, TOWN, SCHOOLS) VALUES ('" + uName + "', '" + birthDate + "', '" + town + "', '" + schools + "')";
            stmt.executeUpdate(dataInput);
            response.sendRedirect("own_page.jsp");
        }else{
            String dataInput = "UPDATE USER1.PERSONAL_INFO SET BIRTH_DATE = '" + birthDate + "', TOWN = '" + town + "', SCHOOLS = '" + schools + "' WHERE USERNAME = '" + uName + "'";
            stmt.executeUpdate(dataInput);
            response.sendRedirect("own_page.jsp");
        }
    }
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            Logger.getLogger(alapServlet.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
